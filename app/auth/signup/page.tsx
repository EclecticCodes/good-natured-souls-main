      ...
      <main className="min-h-screen flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-lg">
          <div className="text-center mb-8">
            ...
            <p className="font-oswald text-gray-500 tracking-widest mt-2">CREATE YOUR ACCOUNT</p>
            <div className="flex items-center justify-center gap-2 mt-4">
              <div className={"w-8 h-1 " + (step >= 1 ? "bg-accent" : "bg-secondaryInteraction")} />
              <div className={"w-8 h-1 " + (step >= 2 ? "bg-accent" : "bg-secondaryInteraction")} />
            </div>
          </div>
            ...
            {step === 1 && (
              <>
                ...
                <form onSubmit={handleNext} className="flex flex-col gap-4">
                  ...
                  <button type="submit" className="bg-accent text-primary font-oswald font-bold text-sm px-6 py-4 tracking-widest hover:bg-accentInteraction transition-colors">NEXT</button>
                </form>
                <div className="text-center mt-6">
                  <a href="/auth/login" className="text-gray-500 hover:text-accent transition-colors font-oswald text-xs tracking-widest">ALREADY HAVE AN ACCOUNT? SIGN IN</a>
                </div>
              </>
            )}
            {step === 2 && (
              <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                <div className="text-center">
                  <div className="w-24 h-24 rounded-full border-2 border-accent mx-auto mb-3 flex items-center justify-center overflow-hidden cursor-pointer hover:opacity-80 transition-opacity" onClick={() => fileRef.current?.click()}>
                    {avatar ? (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {GNS_ARTISTS.map(artist => (
                      <button key={artist} type="button" onClick={() => toggleArray("favoriteArtists", artist)} className={"font-oswald text-xs px-3 py-2 border tracking-widest transition-colors " + (form.favoriteArtists.includes(artist) ? "border-accent text-accent bg-secondaryInteraction" : "border-secondaryInteraction text-gray-500 hover:border-accent")}>
                        {artist}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {GENRES.map(genre => (
                      <button key={genre} type="button" onClick={() => toggleArray("genres", genre)} className={"font-oswald text-xs px-3 py-2 border tracking-widest transition-colors " + (form.genres.includes(genre) ? "border-accent text-accent bg-secondaryInteraction" : "border-secondaryInteraction text-gray-500 hover:border-accent")}>
                        {genre}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="submit" disabled={loading} className="flex-1 bg-accent text-primary font-oswald font-bold text-sm py-4 tracking-widest hover:bg-accentInteraction transition-colors disabled:opacity-50">{loading ? "CREATING..." : "CREATE ACCOUNT"}</button>
                </div>
              </form>
            )}
          </div>
